...
from experiments.ablation.Deepwalk import sample_d
from experiments.ablation.node2vec import sample_n
...
class GraphSage(nn.Module):
    ...
    def forward(self, nodes_batch):
        ...
        #Node2vec
        for index in range(1, self.num_layers + 1):
            aggregate_feats = self.aggregate_n(pre_hidden_embs)
            sage_layer = getattr(self, 'sage_layer' + str(index))
            # self.dc.logger.info('sage_layer.')
            cur_hidden_embs = sage_layer(self_feats=pre_hidden_embs,
                                         aggregate_feats=aggregate_feats)
    ...
    def aggregate_d(self, pre_hidden_embs):
        neigh = sample_d()
        agg_feats = []
        for i in range(len(pre_hidden_embs)):
            if len(neigh[i]) == 0:
                ...
                agg_feat = pre_hidden_embs[neigh[i][0]] * 1/len(neigh[i])
                for j in range(1, len(neigh[i])):
                    agg_feat += pre_hidden_embs[neigh[i][j]] * 1/len(neigh[i])
            agg_feats.append(agg_feat)
    ...
    def aggregate_n(self, pre_hidden_embs):
        neigh = sample_n()
        agg_feats = []
        for i in range(len(pre_hidden_embs)):
            if len(neigh[i]) == 0:
