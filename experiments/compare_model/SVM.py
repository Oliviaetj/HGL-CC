...
import numpy as np
import random
from datasets.graph_gen import SVMset,S1set,S2set,S3set,S0set,Allset
...
if __name__ == '__main__':
    ...
    train = set[:int(len(set) * 0.8)]
    test = set[int(len(set) * 0.8):]
    for subgraph in train:
        X_train = subgraph.ndata['feat']
        y_train = subgraph.ndata['label']
    for subgraph in test:
        X_test = subgraph.ndata['feat']
        y_test = subgraph.ndata['label']
    # X_train, y_train, X_test, y_test = get_datasets()
    # print(X_test,y_test,X_train,y_train)
    svm = SVM(max_iter=200)
    svm.fit(X_train, y_train)
    y_pred = svm.predict0(X_test)
