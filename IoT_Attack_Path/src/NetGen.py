...
import sys
sys.path.append("..")
from IoT_Attack_Path.src.SecurityEvaluator import *
import csv
import os
...
def createCombinedALFH(vuls,target):
    # create a media player and assign two vulnerabilities
    media_player = iot('media_player')
    media_player.subnet.append('wifi')
    v00 = vuls[0][0]
...
def analyseProbability(net_o, net_p, pri):
    pro_original, _, _ = attackProAnalysis(net_o, pri)
    pro_predicted, _, _ = attackProAnalysis(net_p, pri)
...
def analyseImpact(net_o, net_p, pri):
    impact_original, _, _ = attackImpactAnalysis(net_o, pri)
    impact_predicted, _, _ = attackImpactAnalysis(net_p, pri)
...
def analyseRisk(net_o, net_p, pri):
    risk_original, _, _ = riskAnalysis(net_o, pri)
    risk_predicted, _, _ = riskAnalysis(net_p, pri)
...
def analyseBaseScore(net_o, net_p, pri):
    bs_original, _, _,path0 = baseScoreAnalysis(net_o, pri)
    bs_predicted, _, _ = baseScoreAnalysis(net_p, pri)
...
def neigh_basescore():
    ...
    device = ['humidity_Sensor', 'ventilator', 'CO2_sensor', 'heater', 'window_sensor0', 'window_sensor1',
              'air_conditioner', 'CO_sensor', 'light0', 'light1', 'occupancy_sensor', 'thermometer',
              'fire_alarm', 'brightness_sensor', 'motion_sensor', 'smoke_sensor', 'media_player', 'projector',
              'tv', 'speaker', 'light_controller']
    ...
    for node in device:
        # print(node)
        net_o = createCombinedALFH(vuls_original, node)
        bs_original, _, _, path0 = baseScoreAnalysis(net_o, 3)
        # print(path0)
        path1 = []
        # print('[%d]' % (device.index(node)), end='\t')
        ...
        # print('END\n')
        neigh.append(path1)
        basescore.append(bs_original)

    return neigh,basescore
        # print('1111111111111111111111111111111')
        # print(path0, bs_original)
...
if __name__ == '__main__':
    ...
    # net_original = createCombinedALFH(vuls_original,'air_conditioner')
    # net_predicted = createCombinedALFH(vuls_predicted,'air_conditioner')
    ...
    for node in device:
        ...
        # print('1111111111111111111111111111111')
        print(path0,bs_original)
        # print('111111111111111111111111111111111111111111111111111111111111111111111111'+path0)
